...
from data.siamese_dataset import ConsumptionPairDataset, suffix_stem
...
DEFAULT_YOLO_CHECKPOINT = (
    REPO_ROOT
    / "runs/mlflow/593474382547047869/693ece4bf847448cb7ffbb1019c88244/artifacts/weights/best.pt"
)
...
def segment_image(model: Any, image_path: Path, cache_dir: Path, conf: float, imgsz: int) -> list[dict[str, Any]]:
    ...
    cache_path = cache_dir / f"{suffix_stem(image_path.name)}.json"
    cache_path.write_text(json.dumps({"detections": detections}, indent=2), encoding="utf-8")
    return detections
...
def main() -> None:
    ...
    print(f"\nSegmenting pair with: {args.yolo_checkpoint}")
    yolo = YOLO(str(args.yolo_checkpoint))
    before_detections = segment_image(yolo, before_path, cache_dir, args.conf, args.imgsz)
    after_detections = segment_image(yolo, after_path, cache_dir, args.conf, args.imgsz)
