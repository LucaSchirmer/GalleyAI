    ...
    def __init__(
        ...
        for pair in pairs:
            ...
            for metric_name, values in pair.get("numbers", {}).items():
                ...
                # Must exist in the before image.
                if not self.mask_cache.has_any_class(pair["before"], classes):
                    dropped_no_before_mask += 1
                    continue

                # If the item was not completely consumed, it should still be detectable
                # in the after image.
                if (
                    target_pct < 100.0
                    and not self.mask_cache.has_any_class(pair["after"], classes)
                ):
                    dropped_no_after_mask += 1
                    continue
            ...
            for field, values in pair.get("choices", {}).items():
                ...
                # "Not consumed" (=0%) should still be visible afterwards.
                if (
                    target_pct < 100.0
                    and not self.mask_cache.has_any_class(pair["after"], classes)
                ):
        ...
        self.dropped_quality = dropped_quality
        self.dropped_no_before_mask = dropped_no_before_mask
        self.dropped_no_after_mask = dropped_no_after_mask
        
        print(
            f"ConsumptionPairDataset({manifest_path}): {len(self.samples)} samples "
            f"(dropped {self.dropped_quality} quality-flagged pairs, "
            f"{self.dropped_no_before_mask} samples with no 'before' mask, "
            f"{self.dropped_no_after_mask} samples with inconsistent 'after' mask)"
        )
